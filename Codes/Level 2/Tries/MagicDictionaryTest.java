class MagicDictionary {
    ...
    public boolean search(String word, int idx, Node curr, boolean change) {
        if (idx == word.length()) {
            System.out.println(change + " wtf " + curr.isTerminal());
        ...
        // if the word exist in the trie
        if (curr.contains(ch) == true && search(word, idx + 1, curr.get(ch), change)) {
            return true;
        }

        
            // if it doesn't exist replace the wore except itself
            if (change == true)
                return false;
            for (char chn = 'a'; chn <= 'z'; chn++) {
                if (chn == ch)
                    continue;
                // if the current word exist in the trie,
                // mark the word as replaced and furtur check aage trie mai
                // aaga chlna sa kaam hojaye toh
